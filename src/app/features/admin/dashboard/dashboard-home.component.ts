...
@Component({
    ...
    <div class="space-y-6">
        ...
        <!-- Ultimos 7 dias -->
        <div>
            <h2 class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Últimos 7 días</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6">

                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                    <h3 class="text-gray-500 text-sm font-medium">Personas</h3>
                    <p class="text-2xl font-bold text-gray-800">{{ stats()?.visitors_7d ?? '—' }}</p>
                </div>

                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                    <h3 class="text-gray-500 text-sm font-medium">Vistas</h3>
                    <p class="text-2xl font-bold text-gray-800">{{ stats()?.views_7d ?? '—' }}</p>
                </div>

                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                    <h3 class="text-gray-500 text-sm font-medium">Clicks en compras</h3>
                    <p class="text-2xl font-bold text-gray-800">{{ stats()?.clicks_7d ?? '—' }}</p>
                </div>
        ...
        <div class="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-gray-100">
            ...
            <p class="text-xs text-gray-400 mb-6">Últimos 7 días</p>
...
export class DashboardHomeComponent implements OnInit {
    ...
    conversion = computed(() => {
        const s = this.stats();
        if (!s || !s.visitors_7d) return '—';
        return ((s.clicks_7d / s.visitors_7d) * 100).toFixed(1) + '%';
    });
